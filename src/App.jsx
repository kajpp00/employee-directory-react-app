...
function App() {
    ...
    const filteredEmployees = employees.filter((employee) => {
        const matchesLastName = employee.last_name.toLowerCase().includes(debouncedLastNameQuery.toLowerCase());
        const matchesFirstName = employee.first_name.toLowerCase().includes(debouncedFirstNameQuery.toLowerCase());
        const matchesDepartment = debouncedDepartmentFilter
            ? employee.department?.toLowerCase().includes(debouncedDepartmentFilter.toLowerCase())
            : true;
    ...
    return (
        <>
            <div className="App container py-5">
                ...
                <p>
                    To update your contact information in the directory, you will need to log in to Workday. In order for the changes to be reflected in the Campus Directory, information must be updated exactly as indicated in the <a href="https://www.tamuk.edu/finance/its/support/update-campus-directory.html" target="_blank" rel="noopener noreferrer"> instructions</a>.
                    E-mail addresses are maintained by ITS. If your e-mail address is incorrect, please call the ITS HelpDesk at <a href="tel:3615934357">(361) 593-4357</a>.
                </p>
                ...
                <div className="row justify-content-end align-content-center">
                    ...
                    <div className="col-auto">
                        <button className="btn btn-sm btn-secondary" onClick={resetFilters} aria-label="Reset filters">
                            Reset Filters
                        </button>
                    </div>
                </div>
                ...
                <div className="row employee-wrapper">
                    {currentEmployees.length === 0 ? (
                        <div className="col-12 my-2 p-3 rounded">
                            <h2 role="alert">No results found! Try adjusting your search criteria.</h2>
                        </div>
                    ) : (
                        ...
                        currentEmployees.map((employee, index) => (
                            ...
                            className="col-12 my-2 p-3"
                                ...
                                exit={{ opacity: 0, y: -10 }}
                                transition={{
                                    delay: employees.indexOf(employee) * 0.03
                                }}
                            >
                                <div className="row">
                                    <div className="col">
                                        <img className='img-fluid shadow-sm mb-4'
                                            src={employee.photo_url ? employee.photo_url : 'https://www.tamuk.edu/_images_sitewide/_campus_repository/no-img.jpg'}
                                            alt={employee.photo_url ? `${employee.first_name} ${employee.last_name} Headshot` : 'default image'}
                                        />
                                    </div>
                                    <div className="col-10">
                                        <h2 className="h4">
                                            {employee.last_name}{employee.suffix ? ` ${employee.suffix}, ` : ','} {employee.first_name} {employee.middle_name}
                                        </h2>
                                        <div className="details ml-md-2">
                                            <p>
                                                <strong>Phone: </strong>
                                                {employee.phone ? (
                                                    <a href={`tel:${employee.phone}`} aria-label={`Call ${employee.phone}`}>
                                                        {employee.phone}
                                                    </a>
                                                ) : "N/A"}
                                            </p>
                                            <p>
                                                <strong>Email: </strong>
                                                {employee.email ? (
                                                    <a href={`mailto:${employee.email}`} aria-label={`Email ${employee.email}`}>
                                                        {employee.email}
                                                    </a>
                                                ) : "N/A"}
                                            </p>
                                            <p><strong>Title: </strong> {employee?.position || "N/A"}</p>

                                            {employee.department === employee.division_description ? (
                                                <p><strong>Organization: </strong> {employee.division_description}</p>
                                            ) : (
                                                <p><strong>Organization: </strong> {employee.department} | {employee.division_description}</p>
                                            )}
                                        </div>
                                    </div>
                                </div>
